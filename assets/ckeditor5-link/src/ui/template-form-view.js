...
export default class TemplateFormView extends View {
	...
	constructor( locale, linkCommand ) {
		 ...
		 * @member {module:ui/labeledfield/labeledfieldview~LabeledFieldView}
		 */
		this.typeInputView = this._createDropDownInput('Template Variable Type');/**
	 ...
	 * @returns {module:ui/labeledfield/labeledfieldview~LabeledFieldView} Labeled field view instance.
	 */
	_createDropDownInput(label) {
		const createLabeledDropdown = ( labeledFieldView, viewUid, statusUid ) =>{

			const dropdownView = createDropdown( labeledFieldView.locale , SplitButtonView );
			...
			buttons.push( this._createDropdownButton( 'Short text' ));

			// Add another component to the array of toolbar items.
			buttons.push( this._createDropdownButton(  'Long text' ) );
