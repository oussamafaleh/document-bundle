...
export default class TemplateUi extends Plugin {
	...
	_createActionsView() {
		const editor = this.editor;
		const actionsView = new TemplateActionsView( editor.locale );
	...
	_createFormView() {
		...
		const formView = new TemplateFormView( editor.locale, templateCommand );

		formView.labelInputView.fieldView.bind( 'value' ).to( templateCommand, 'value' );

		// Form elements should be read-only when corresponding commands are disabled.
		formView.labelInputView.bind( 'isReadOnly' ).to( templateCommand, 'isEnabled', value => !value );

		formView.saveButtonView.bind( 'isEnabled' ).to( templateCommand );

		// Execute link command after clicking the "Save" button.
		this.listenTo( formView, 'submit', () => {
			const {label}  = formView.typeInputView.fieldView.buttonView;
			const templateVar = {
	 ...
	 */
	_closeFormView() {
		const templateCommand = this.editor.commands.get( 'template' );
